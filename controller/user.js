...
module.exports = {
    userRegister: async (req, res, next) => {
        try {
            ...
            res.status(200).json({message: "User registerd successfully, kindly verify your mail", success: true, response: {
                _id:newUser._id,
                name:newUser.name,
    ...
    deleteUser: async(req,res,next)=>{
        try{
            ...
            return res.status(200).json({success:true, message:"User deleted successfully", response:user._id})
        }
        catch(error){
            return res.status(500).json({success:false, message:"Internal Server Error", error: error.message})
        }
    ...
    sendOTP: async (req, res, next)=> {
            try {
                ...
                if (!email) {
                    return res.status(200).json({success:false, message:"Fields are empty"})
                }
                const OTP = Math.floor(100000 + Math.random() * 900000)
                ...
                user.otp = Number(OTP)
                await user.save()
                return res.status(200).json({success:true, message:`OTP has been sent to email, it will be only valid for 5 minutes`})
            }
            catch (error) {
                return res.status(500).json({success:false, message:"Internal Server Error", error: error.message})
            }
    ...
    verifyOTP: async( req, res, next)=>{
        try {
            ...
            if(!newPassword || !otp || !email){
                return res.status(404).json({success:false, message:"Fields are empty"})
            }
            const user = await User.findOne({ email });
            if (!user){
            ...
            if (user.otp === -1) {
                return res.status(400).json({success:false, message:"Given OTP is expired"})
            }
            ...
            user.otp = -1
            await user.save()
            return res.status(200).json({success:true, message:"Password has been change successfully", response:user})
        }
        catch(error){
            return res.status(500).json({success:false, message:"Internal Server Error", error: error.message})
        }
    ...
    updateProfilePicture: async(req, res,next)=>{
        try{
            ...
            await user.save()
            return res.status(200).json({success:true, message:"Profile picture updated successfully", response:user})
        }
        catch(error){
            return res.status(500).json({success:false, message:"Internal Server Error", error: error.message})
        }
    ...
    getUser: async (req, res, next) => {
        try {
            ...
            if(! user){
                return res.status(404).json({success:false, message:"User not found", response:{}})
            }
            return res.status(200).json({success:true, message:"User found", response:user})
        }
        catch (error) {
            return res.status(500).json({ success: false, message: "Internal Server Error", error: error.message })
        }
    ...
    updateUser: async(req, res, next)=>{
        try{
            ...
            await user.save()
            return res.status(200).json({success:true, message:"User updated successfully", response:user})
        }
        catch(error){
            return res.status(500).json({ success: false, message: "Internal Server Error", error: error.message })
        }
    ...
    searchUserByName: async(req,res,next)=>{
        try{
            ...
            return res.status(200).json({success:true, message:"User found successfully", response:users})
        }
        catch(error){
            return res.status(500).json({ success: false, message: "Internal Server Error", error: error.message })
        }
